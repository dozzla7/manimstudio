...
try {
  if (process.env.S3_ACCESS_KEY_ID && process.env.S3_SECRET_ACCESS_KEY) {
    ...
    console.warn('⚠️ S3 credentials missing. Skipping S3 Client initialization.');
  }
} catch (error) {
...
export async function uploadToS3(filePath: string, objectKey: string): Promise<string | null> {
  ...
  try {
    ...
    // URL is usually https://my-bucket.t3.storageapi.dev/key
    
    // Extract domain from endpoint (e.g. https://t3.storageapi.dev -> t3.storageapi.dev)
    const endpointUrl = new URL(ENDPOINT);
    const host = endpointUrl.host;
    
    const publicUrl = `https://${BUCKET_NAME}.${host}/${objectKey}`;
    return publicUrl;
  } catch (error) {
